    ...
    func downloadChannelData(username: String)
    {
        ...
        let task = session.dataTaskWithRequest(request) { (data, response, error) ->
            ...
            do
            {
                ...
                let contentDetails = firstItem["contentDetails"] as! Dictionary<NSObject, AnyObject>
                let relatedPlaylists = contentDetails["relatedPlaylists"] as! Dictionary<NSObject, String>
                
                YoutubeManager.sharedInstance.selectedPlaylist = relatedPlaylists["uploads"]
